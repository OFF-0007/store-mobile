...
export default function TaxesScreen() {
  ...
  async function loadData() {
    try {
      // Assuming endpoint is /taxes or /tax-rates based on typical setups
  ...
  async function handleSave() {
    ...
    try {
      setIsSaving(true);
      await apiClient.post('/tax-rates', form);
      ...
      {isModalOpen && (
        <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
          <View className="bg-white rounded-3xl p-6 shadow-2xl">
            <Text className="text-lg font-black text-slate-800 uppercase mb-4">Add Tax Rate</Text>
