...
export default function AddCustomerScreen() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      Alert.alert("Error", e.response?.data?.message || e.message || "Failed to add customer");
    } finally {
      setIsSaving(false);
    }
  }
    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={[]}>
      ...
      <ScrollView className="flex-1 p-4" keyboardShouldPersistTaps="handled">
        <GlassCard className="p-4 gap-4">
          <View>
            <Text className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Customer Name *</Text>
            <TextInput
              value={form.name}
