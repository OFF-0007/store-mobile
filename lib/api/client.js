...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    // IMPORTANT: We attach the message to the original axios error so that
    // callers can still inspect error.response.data (e.g. for validation details).
    const data = error.response?.data;
    
    // Handle 401 Unauthenticated globally
    if (error.response?.status === 401) {
      // Clear secure storage manually in case of circular dependency
      SecureStore.deleteItemAsync(TOKEN_KEY).catch(() => {});
      SecureStore.deleteItemAsync("user_session").catch(() => {});
      
      try {
        // Dynamically import to avoid circular dependency with authStore
        const { useAuthStore } = require("@/store/authStore");
        const store = useAuthStore.getState();
        if (store && typeof store.logout === 'function') {
           // update state directly if possible
    ...
    // Attach readable message but keep the original error (preserves .response)
