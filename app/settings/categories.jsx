    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={[]}>
      <View style={{
        ...
        <View style={{ alignItems: 'center', flex: 1 }}>
          <Text style={{ color: '#fff', fontSize: 15, fontWeight: '900', letterSpacing: 1, textTransform: 'uppercase' }}>Categories</Text>
          <Text style={{ color: 'rgba(255,255,255,0.8)', fontSize: 9, fontWeight: '700', letterSpacing: 0.5, marginTop: 1 }}>Manage product categories</Text>
        </View>
      ...
      {isLoading ? (
        <View className="flex-1 justify-center items-center"><ActivityIndicator size="large" color="#d946ef" /></View>
      ) : (
        <FlatList
          data={data}
          ...
          renderItem={({ item }) => (
            <GlassCard className="p-4 flex-row items-center justify-between">
              ...
              <Ionicons name="grid-outline" size={20} color="#cbd5e1" />
            </GlassCard>
          )}
          ListEmptyComponent={<Text className="text-center text-slate-400 font-bold mt-10">No categories found. Click + to add one.</Text>}
        />
      )}
      ...
      {isModalOpen && (
        <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
          <View className="bg-white rounded-3xl p-6 shadow-2xl">
            <Text className="text-lg font-black text-slate-800 uppercase mb-4">Add Category</Text>
            <Text className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Category Name *</Text>
            <TextInput
              value={form.name}
              onChangeText={(text) => setForm({ name: text })}
              className="bg-slate-50 border-2 border-slate-200 rounded-2xl px-4 py-3.5 text-slate-800 text-sm font-bold focus:border-fuchsia-400 mb-6"
            ...
            <View className="flex-row gap-4">
              ...
              <TouchableOpacity onPress={handleSave} disabled={isSaving} className="flex-1 py-4 bg-fuchsia-500 rounded-2xl items-center flex-row justify-center">
