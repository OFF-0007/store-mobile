...
export default function UnitsScreen() {
  ...
  async function handleSave() {
    if (!form.name.trim() || !form.short_name.trim()) {
      Alert.alert("Validation Error", "Name and Short Name are required");
      return;
    }
    try {
      setIsSaving(true);
        ...
        <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
          <View className="bg-white rounded-3xl p-6 shadow-2xl">
            ...
            <TextInput
              ...
              className="bg-slate-50 border-2 border-slate-200 rounded-2xl px-4 py-3.5 text-slate-800 text-sm font-bold focus:border-yellow-400 mb-4"
              placeholder="e.g., Kilogram, Piece"
            />

            <Text className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 ml-1">Short Name *</Text>
            <TextInput
              value={form.short_name}
              onChangeText={(text) => setForm({ ...form, short_name: text })}
              className="bg-slate-50 border-2 border-slate-200 rounded-2xl px-4 py-3.5 text-slate-800 text-sm font-bold focus:border-yellow-400 mb-6"
            ...
            <View className="flex-row gap-4">
              ...
              <TouchableOpacity onPress={handleSave} disabled={isSaving} className="flex-1 py-4 bg-yellow-500 rounded-2xl items-center flex-row justify-center">
