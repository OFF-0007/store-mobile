...
import { generateModernA4HTML, generateClassicA4HTML } from "../../utils/pdfTemplates";
...
export default function POSScreen() {
  ...
  const handleDownloadA4Invoice = async (sale, layoutType = 'modern') => {
    if (!sale) return;

    // Fetch store settings to populate invoice header (name, address, GST, phone)
    let settings = {};
    try {
      const res = await apiClient.get('/settings');
      if (res.data) settings = res.data;
    } catch (e) {
      console.warn('Could not load settings for invoice', e.message);
    }

    const html = layoutType === 'classic'
      ? generateClassicA4HTML(sale, products, true, settings)
      : generateModernA4HTML(sale, products, true, settings);
        ...
        <SafeAreaView className="flex-1 bg-slate-100 justify-between">
          ...
          <View className="p-6 bg-white border-t-2 border-slate-200" style={{ gap: 12 }}>
            ...
            <View style={{ flexDirection: "row", justifyContent: "space-between", width: "100%", gap: 12 }}>
              <TouchableOpacity
                onPress={() => handleDownloadA4Invoice(completedSale, 'modern')}
                activeOpacity={0.85}
                style={{ flex: 1, paddingVertical: 18, backgroundColor: "#1e293b", borderRadius: 24, flexDirection: "row", alignItems: "center", justifyContent: "center", elevation: 4, shadowColor: "#1e293b", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 }}
              >
              ...
              <TouchableOpacity
                onPress={() => handleDownloadA4Invoice(completedSale, 'classic')}
                activeOpacity={0.85}
                style={{ flex: 1, paddingVertical: 18, backgroundColor: "#334155", borderRadius: 24, flexDirection: "row", alignItems: "center", justifyContent: "center", elevation: 4, shadowColor: "#334155", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 }}
              >
                <Ionicons name="document-text-outline" size={16} color="#fff" style={{ marginRight: 6 }} />
                <Text className="text-white font-black text-xs uppercase tracking-widest">
                  Classic A4
