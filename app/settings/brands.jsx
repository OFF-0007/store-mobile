...
export default function BrandsScreen() {
  ...
  async function handleSave() {
    ...
    try {
      setIsSaving(true);
      if (editId) {
        await apiClient.put(`/brands/${editId}`, form);
      } else {
  ...
  function handleDeleteConfirm(id) {
    Alert.alert("Confirm Delete", "Are you sure you want to delete this brand?", [
      ...
      {isModalOpen && (
        <View style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', padding: 20 }}>
          <View className="bg-white rounded-3xl p-6 shadow-2xl">
            <Text className="text-lg font-black text-slate-800 uppercase mb-4">{editId ? "Edit Brand" : "Add Brand"}</Text>
